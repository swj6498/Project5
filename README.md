...
이 프로젝트는 웹 크롤링 기술을 활용한 로봇이 KOSPI/KOSDAQ 종목의 실시간 시세와 국내·해외 관련 뉴스를 자동 수집하고, 수집된 데이터들을 가공하여 사용자에게 한눈에 보여줍니다.
...
<details>
<summary><strong>📰 국내/해 뉴스 크롤링 (네이버 뉴스)</strong></summary>
...
<details> 
<summary><strong>🧠 형태소 분석 기반 검색 데이터 구축</strong></summary>

뉴스 기사 등록 시 형태소 분석을 통해 핵심 명사를 추출하고,
이를 기반으로 검색에 활용 가능한 키워드 데이터를 구성합니다.

- KoNLPy(Okt) 형태소 분석기 사용
- 불용어 제거를 통한 키워드 정제
- 검색·오타 교정·추천 기능의 기반 데이터로 활용

</details> 

<details> 
<summary><strong>📊 TF-IDF 기반 검색 랭킹</strong></summary>

TF-IDF와 코사인 유사도를 활용해
검색어와 뉴스 간의 관련도를 계산하고 결과를 정렬합니다.

- 제목/본문 분리 후 가중치 적용
- 키워드 포함 여부를 반영한 점수 보정
- 단순 최신순이 아닌 정확도 중심 검색 결과 제공

</details> 

<details> 
<summary><strong>✏️ 검색어 오타 교정</strong></summary>

사용자가 오타를 입력해도 검색이 가능하도록
유사 검색어를 자동으로 보정하는 기능입니다.

- 정확 일치 여부 우선 확인
- 유사도(Levenshtein 거리) 기반 후보 탐색
- 필요 시 LLM을 활용한 보정 처리
...
부분 일치 자동완성 검색어를 제공합니다.

- regex(query, "i")를 이용한 대소문자 구분 없는 부분 일치 검색
- 결과 개수 limit(10)으로 제한
- 조회된 문서에서 검색어 문자열만 추출하여 반환
